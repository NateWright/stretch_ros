...
int main(int argc, char *argv[])
{
    ...
    //Setup
    static const std::string PLANNING_GROUP = "armgrip";
    ...
    //Visualization
    moveit_visual_tools::MoveItVisualTools visual_tools("base_link", rvt::RVIZ_MARKER_TOPIC, moveit_cpp_ptr->getPlanningSceneMonitor());
    ...
    planning_components->setGoal(target_pose1, "link_grasp_center");
    ...
    start_pose.position.y = start_pose.position.y;
    start_pose.position.z = start_pose.position.z;
    ...
    if (plan_solution2)
    {
        ...
        visual_tools.publishText(text_pose, "Start Pose", rvt::WHITE, rvt::XLARGE);
        visual_tools.publishAxisLabeled(robot_state.getGlobalLinkTransform("base_link"), "start_pose");
        visual_tools.publishText(text_pose, "Goal Pose", rvt::WHITE, rvt::XLARGE);
        visual_tools.publishAxisLabeled(target_pose1.pose, "target_pose");
