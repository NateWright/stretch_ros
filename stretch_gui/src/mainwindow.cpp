...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(this, &MainWindow::homeRobot, moveItNode_, &StretchMoveItInterface::homeRobot);        // Server only
    connect(this, &MainWindow::cameraSetTilt, moveItNode_, &StretchMoveItInterface::headSetTilt);  // Server only

    connect(ui->ButtonBack, &QPushButton::clicked, this, &MainWindow::changeToPage1);  // Both

    connect(ui->CameraMoveButtonUp, &QPushButton::clicked, moveItNode_, &StretchMoveItInterface::headUp);        // Client to server
    connect(ui->CameraMoveButtonDown, &QPushButton::clicked, moveItNode_, &StretchMoveItInterface::headDown);    // Client to server
    connect(ui->CameraMoveButtonLeft, &QPushButton::clicked, moveItNode_, &StretchMoveItInterface::headLeft);    // Client to server
    connect(ui->CameraMoveButtonRight, &QPushButton::clicked, moveItNode_, &StretchMoveItInterface::headRight);  // Client to server
    connect(ui->CameraMoveButtonHome, &QPushButton::clicked, moveItNode_, &StretchMoveItInterface::headHome);    // Client to server

    // Find point in Camera
    connect(ui->DisplayCamera, &SceneViewer::mouseClick, cameraSub_, &RosCamera::sceneClicked);      // Client to server
    ...
    connect(cameraSub_, &RosCamera::clickInitiated, ui->PointPleaseWait, &QTextBrowser::show);  // Sever to client

    // Camera feed
    connect(cameraSub_, &RosCamera::imgUpdate, ui->DisplayCamera, &SceneViewer::setCamera);  // Server to client
    // Error: Displays if NaN point was selected
    connect(cameraSub_, &RosCamera::clickFailure, ui->ErrorNanPoint, &QTextBrowser::show);    // Server to client
    connect(cameraSub_, &RosCamera::clickFailure, ui->PointPleaseWait, &QTextBrowser::hide);  // Server to client

    connect(cameraSub_, &RosCamera::checkPointInRange, mapSub_, &MapSubscriber::checkPointInRange);  // Server only

    // True
    connect(mapSub_, &MapSubscriber::validPoint, this, &MainWindow::changeToPage3);  // Both
    // False
    connect(mapSub_, &MapSubscriber::invalidPoint, ui->ErrorOutOfRange, &QTextBrowser::show);  // Server to client
    connect(mapSub_, &MapSubscriber::invalidPoint, ui->PointPleaseWait, &QTextBrowser::hide);  // Server to client

    // Page 3

    ui->DisplayImage->setScaledContents(false);

    connect(ui->ConfirmButtonNo, &QPushButton::clicked, this, &MainWindow::changeToPage2);   // Client to Both
    connect(ui->ConfirmButtonYes, &QPushButton::clicked, this, &MainWindow::changeToPage4);  // Client to Both
    connect(ui->ConfirmButtonYes, &QPushButton::clicked, graspNode_, &GraspNode::lineUp);    // Client to server

    connect(cameraSub_, &RosCamera::imgUpdateWithPoint, ui->DisplayImage, &SceneViewer::setCamera);  // Server to ui

    // Page 4

    ui->DisplayGrasp->setScaledContents(false);

    connect(ui->ButtonBack_2, &QPushButton::clicked, graspNode_, &GraspNode::home);                  // Client to Server
    connect(ui->ButtonBack_2, &QPushButton::clicked, this, &MainWindow::changeToPage2);              // Client to Both
    connect(ui->ButtonReturnObject, &QPushButton::clicked, graspNode_, &GraspNode::returnObject);    // Client to server
    connect(ui->ButtonRelease, &QPushButton::clicked, graspNode_, &GraspNode::releaseObject);        // Client to server
    connect(ui->ButtonReplaceObject, &QPushButton::clicked, graspNode_, &GraspNode::replaceObject);  // Client to server
    connect(ui->ButtonNavigate, &QPushButton::clicked, this, &MainWindow::changeToPage5);            // Client to both

    connect(cameraSub_, &RosCamera::imgUpdate, ui->DisplayGrasp, &SceneViewer::setCamera);  // Server to client
    ...
    connect(graspNode_, &GraspNode::graspDone, ui->ButtonReturnObject, &QPushButton::setEnabled);  // Server to client
